...
class ListSearchView(APIView):
    ...
    def post(self, request, format=None):
        ...
        # Check if something input happened in the search
        if len(search) == 0:
            # show all products if there is no input in the search
            search_results = Product.objects.order_by('-date_created').all()
        else:
            # If there is a search criteria, we filter with said criteria using Q
        ...
        # if the category has a parent, filter only by the category and not the parent as well
        if category.parent:
            ...
            # if this parent category has no children, filter only the category
